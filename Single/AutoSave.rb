...
#------------------------------------------------------------------------------
# * 简介
#  在很多解谜、生存、养成类游戏中，不需要玩家主动存档，而是由系统自动保存进度
# 这时就可以使用本脚本
#  你可以设定脚本，使每次场所移动时自动存档（记录场景）
#  也可以通过下面的事件脚本，配合其他时间系统使用（记录时间）
#  根据你的设定，还可以做其他用途
...
#+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=#
    ...
    # * 用户设置
    #------------------------------------------------------------------------
    def self.function(type, value = nil)
      case type
      when :save
...
class << DataManager
  ...
  _def_ :make_filename, :v do |old, *args|
    ...
    "MoAutoSave" + old.match(/\.[^\.]*$/)[0] : old
  end
  #--------------------------------------------------------------------------
  # * 存游戏前先存设置
  #--------------------------------------------------------------------------
  _def_ :save_game, :b do |*args|
    mo_autosave_save_sets
  end
  #--------------------------------------------------------------------------
  # * 自动保存
  #--------------------------------------------------------------------------
  ...
  def mo_auto_load_game
    $mo_autosave[:Saving] = true
    _ = Dir.glob(make_filename($mo_autosave[:SaveID])).empty? ? false : 
